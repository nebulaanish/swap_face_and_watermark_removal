...
def face_swap(image_url, face_url):
    if settings.MODE == "prod":  # To save api calls
        ...
        headers = {
            ...
            "X-RapidAPI-Key": settings.FACESWAP_API_KEY,
            "X-RapidAPI-Host": "faceswap-image-transformation-api.p.rapidapi.com",
        }

        payload = {"TargetImageUrl": image_url, "SourceImageUrl": face_url}
...
def create_mask(result_url, result_path, mask_path):
    ...
    vertices_array = np.array(
        [(point["x"], point["y"]) for point in vertices], dtype=np.int32
    )
...
def detect_logo(result_path):
    ...
    headers = {"Authorization": f"Bearer {settings.EDENAI_API_KEY}"}
    url = "https://api.edenai.run/v2/image/logo_detection"
    data = {"providers": "google", "fallback_providers": ""}
